	...
	m_jumpRechargeTimer(0.0f), m_jumpRechargeTime(10.0f), m_stepHeight(stepHeight)
{
	...
	m_pMotionState = new btDefaultMotionState(btTransform(btQuaternion(1.0f, 0.0f, 0.0f, 0.0f).normalized(), btVector3(spawnPos.x, spawnPos.y, spawnPos.z)));
...
void DynamicCharacterController::UpdatePosition()
{
	...
	m_pPhysicsWorld->rayTest(m_pRigidBody->getWorldTransform().getOrigin(), m_pRigidBody->getWorldTransform().getOrigin() - btVector3(0.0f, m_bottomYOffset + m_stepHeight, 0.0f), rayCallBack_bottom);
